...
class SACAgent(rl_agent.RLAgent):
    ...
    def eval_critic(self, s, a):
        ...
        parallel_vs = self._sess.run(self._parallel_norm_critic_tfs, feed_dict=feed)
        seq_vs = self._sess.run(self._seq_norm_critic_tfs, feed_dict=feed)

        print("norm diff forward pass:", np.linalg.norm(parallel_vs - seq_vs, axis=-1))
    ...
    def _build_nets(self):
        ...
        self._s_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, s_size], name="s"
        )
        self._a_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, a_size], name="a"
        )
        self._tar_val_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None], name="tar_val"
        )
        self._r_ph = tf.compat.v1.placeholder(tf.float32, shape=[None], name="r")
        self._next_s_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, s_size], name="next_s"
        )
        self._terminate_ph = tf.compat.v1.placeholder(
            tf.int32, shape=[None], name="terminate"
        )
        ...
        with tf.compat.v1.variable_scope(self.MAIN_SCOPE):
            ...
            self._sample_norm_a_tfs = [
                self._norm_a_pd_tf.sample() for _ in range(self._num_action_samples)
            ]
        ...
        self._sync_tar_vars_op = list(
            map(
                ...
                zip(
                    tar_critic_vars + tar_actor_vars, main_critic_vars + main_actor_vars
                ),
            )
        )
        self._update_critic_tar_vars_op = list(
            map(
                lambda v: v[0].assign(
                    (1.0 - self._tar_stepsize) * v[0] + self._tar_stepsize * v[1]
                ),
    ...
    def _build_dataset(self, s_size, a_size):
        self._all_s_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, s_size], name="all_s"
        )
        self._all_a_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, a_size], name="all_a"
        )
        self._all_r_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None], name="all_r"
        )
        self._all_next_s_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, s_size], name="all_next_s"
        )
        self._all_terminate_ph = tf.compat.v1.placeholder(
            tf.int32, shape=[None], name="all_terminate"
        )
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(
            (
                ...
                self._all_r_ph,
                self._all_next_s_ph,
                self._all_terminate_ph,
            )
        )
        train_dataset = train_dataset.repeat().shuffle(
            buffer_size=self._replay_buffer._buffer_size, reshuffle_each_iteration=True
        )
        num_procs = 1
    ...
    def _build_losses(self):
        ...
        next_val_tf = tf.compat.v1.where(
            ...
            val_fail * tf.ones_like(next_val_tf),
            next_val_tf,
        )
        ...
        self._critic_loss_tf = 0.5 * tf.reduce_mean(
            input_tensor=tf.reduce_sum(input_tensor=tf.square(norm_val_diff), axis=0)
        )
        self._actor_loss_tf = -tf.reduce_mean(input_tensor=self._sample_norm_critic_tf)

        if self._actor_bound_loss_weight != 0.0:
            self._actor_loss_tf += (
                self._actor_bound_loss_weight
                * self._action_bound_loss(self._norm_a_pd_tf)
            )
    ...
    def _build_solvers(self):
        ...
        actor_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self._actor_stepsize)

        if self._use_MPI_solver:
            self._critic_grad_tf = tf.gradients(ys=self._critic_loss_tf, xs=critic_vars)
            self._critic_solver = mpi_solver.MPISolver(
                self._sess, critic_opt, critic_vars
            )

            self._actor_grad_tf = tf.gradients(ys=self._actor_loss_tf, xs=actor_vars)
            self._actor_solver = mpi_solver.MPISolver(self._sess, actor_opt, actor_vars)
        else:
            self._critic_train_op = critic_opt.minimize(
                self._critic_loss_tf, var_list=critic_vars
            )
            self._actor_train_op = actor_opt.minimize(
                self._actor_loss_tf, var_list=actor_vars
            )
    ...
    def _build_net_actor(self, net_name, input_tfs, reuse=False):
        with tf.compat.v1.variable_scope(self.ACTOR_SCOPE, reuse=reuse):
            h = net_builder.build_net(
                net_name=net_name, input_tfs=input_tfs, reuse=reuse
            )
    ...
    def _build_net_critic(self, net_name, input_tfs, num_nets, reuse=False):
        should_squeeze = len(input_tfs[0].shape) == 2 and self._parallel_ensemble
        out_size = 1
        norm_val_tfs = []
    ...
    def _collect_init_samples(self, max_samples):
        ...
        while sample_count < max_samples:
            ...
            if sample_count >= next_benchmark:
                print(
                    "Collected {} initial samples in {} sec".format(
    ...
    def _update(self, iter, new_sample_count):
        ...
        critic_steps = int(
            np.ceil(self._critic_steps * new_sample_count / self._critic_batch_size)
        )
        critic_info = self._update_critic(critic_steps, self._input_op)

        actor_steps = int(
            np.ceil(self._actor_steps * new_sample_count / self._actor_batch_size)
        )
    ...
    def _update_critic(self, steps, input_op):
        ...
        for b in tqdm.trange(steps):
            ...
            curr_info = self._step_critic(
                s=s, a=a, r=r, next_s=next_s, terminate=terminate
            )
